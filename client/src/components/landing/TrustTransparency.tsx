  ...
  const points = [
    {
      ...
      description:
        'Civic Trust evaluates complaints to provide evidence signals for ward engineers. Automated checks never dismiss or finalize a complaint without administrative oversight.',
    },
    {
    ...
    {
      icon: <ShieldAlert className="w-5 h-5 text-brand-teal-700" />,
    ...
    <section className="py-16 sm:py-20 bg-white border-b border-brand-slate-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-5xl mx-auto">
          {points.map((pt) => (
            <Card key={pt.title} className="border-brand-slate-200">
              <CardBody className="p-6 flex items-start gap-4">
                <div className="w-10 h-10 rounded-xl bg-brand-slate-100 flex items-center justify-center shrink-0 mt-0.5">
                  {pt.icon}
                </div>
                <div>
                  <h3 className="text-base font-bold text-brand-slate-900 mb-1.5">
                    {pt.title}
                  </h3>
